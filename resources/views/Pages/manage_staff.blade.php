            ...
            <div class="col-xxl">
                <div class="card mb-4">
                    <div class="card-header d-flex align-items-center justify-content-between">
                        <h5 class="mb-0">{{ isset($user) ? 'Edit Staff' : 'Add New Staff' }}</h5>
                    </div>
                    <div class="card-body">
                        <form id="unitForm">
                            @csrf
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10">
                                    ...
                                    <div class="invalid-feedback"> Please enter a staff name.</div>
                                </div>
                            </div>
                            <div class="row mb-3">
                                <label class="col-sm-2 col-form-label" for="basic-default-name">Phone Number</label>
                                <div class="col-sm-10">
                                    <input type="text" id="basic-default-name" class="form-control" name="phone" value="{{ isset($user) ? $user->phone : '' }}">
                                </div>
                            </div>
                            <div class="row mb-3">
                                <label class="col-sm-2 col-form-label" for="basic-default-name">Email</label>
                                <div class="col-sm-10">
                                    <input type="email" id="bs-validation-email" name="email" class="form-control email"  value="{{ isset($user) ? $user->email : '' }}" />
                                    <div class="invalid-feedback" id="email-feedback"> Please enter a valid email address.</div>
                                </div>
                            </div>
                            <div class="row mb-3">
                                <label class="col-sm-2 col-form-label" for="basic-default-password42">{{ isset($user) ? 'Create new' : 'Password' }}</label>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#saveUnit').on('click', function(event) {
                ...
                $.ajax({
                    url: "{{ isset($user) ? route('staffs.update', ['id' => $user->id]) : route('staffs.store') }}",
